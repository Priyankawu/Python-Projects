...
def center_window(self,w,h):
    ...
    #calculate x and y coordinates to paint the app centered on teh user's screen
...
#=========================================================
def create_db(self):
    conn = sqlite3.connect('phonebook.db')    #creates the db
    with conn:
        cur = conn.cursor()
        ...
        # You must commit() to save changes and close the databse conneciton
        conn.commit()
    conn.close()
    first_run(self)
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname, \
                        col_phone,col_email) VALUES (?,?,?,?,?)""",('John','Doe','John Doe','111-111-1111','jdoe@email.com'))
            conn.commit()
    conn.close()

def count_records(cur):
    count = ""
    cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""") #counts no. of rows/records from table
    count = cur.fetchone()[0] # returns the row from the table- fetchone(), fetchall() etc.
    return cur,count
...
def onSelect(self,event):
    ...
    with conn:
        ...
        for data in varBody:
            ...
            self.txt_email.insert(1,data[3])
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # check count ot ensure that this is not the last record in
        # the database... cannot delete last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0] #to check if db is empty
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) #call the function to clear all of the textboxes and the selecterd index of listbox
                #onRefresh(self) # update the listbox of hte changes
                conn.commit()
        else:
...
# deletes everything, no exceptions
def onClear(self):
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
...
def onUpdate(self):
    try:
        ...
        return
    #The user will onlay allowed to update changes for phone and emails.
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): #ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 == 0: #if proposed changes are no already in the databse, then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}) and ({}) will be implemented for ({}). \n".format(var_phone,var_email,var_value))
                print(response)
                if response:
                    with conn:
                        cursor = conn.cursor()
